...
require(TARGET, "local exact = raycastEntity(screenPosition)", "pointer coordinates must stay aligned with the rendered viewport")
require(TARGET, "findForgivingScreenCandidate(screenPosition)", "forgiving selection must use the same pointer coordinate space as exact raycasts")
...
require(TARGET, "localGuiBlocksPointer", "processed input must only suppress targeting when Luna UI actually owns the pointer")
require(TARGET, "GetGuiObjectsAtPosition", "targeting must inspect local GUI ownership for processed clicks")
require(TARGET, "gameProcessed and localGuiBlocksPointer(screenPosition)", "desktop processed clicks must remain targetable off interactive UI")
require(TARGET, "gameProcessed and localGuiBlocksPointer(position)", "touch processed taps must remain targetable off interactive UI")
...
require(BAR, 'slotCount = if UserInputService.TouchEnabled then 6 else 10', "skill bar must expose six mobile and ten desktop slots")
require(BAR, 'for index = 1, slotCount do', "skill bar must build the platform slot count")
