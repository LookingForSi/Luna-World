...
- При обычном входе starter-repair больше не переэкипирует учебное оружие поверх уже надетого подходящего классу No-Grade оружия; старые несовместимые профили (например, Следопыт с учебным мечом) по-прежнему автоматически исправляются.
...
- Поле количества в продаже стало интерактивным: изменение уже выбранной позиции пересчитывает корзину, Enter переносит/обновляет позицию без обязательного клика по стрелке.
