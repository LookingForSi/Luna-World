...
Общая multiplayer PvE-зона уровней 1–14; уровень 15 зарезервирован под кульминацию Ruins of Selene.
...
Финальный boss уровня 15 (рабочее название: **Selene's Fallen Guardian**) должен иметь минимум три читаемых состояния / механики:
...
### Ресурсы персонажа в HUD

Статус персонажа содержит четыре отдельные строки:

- `CP` — PvP-only Combat Points; в PvE не поглощает урон и пока является зарезервированным слоем для будущей PvP-механики;
- `HP` — здоровье;
- классовый ресурс: `STM` для Рыцаря, `FCS` для Следопыта, `MP` для Мистика;
- `XP` — опыт.

`FCS / Focus` намеренно остаётся отдельным типом ресурса Следопыта. Даже если ранний vertical slice восстанавливает его похоже на Stamina, отдельный тип оставляет возможность позднее балансировать дальний класс собственными правилами без изменения Рыцаря.
...
### 5.2. Бой с выбором цели

Бой сохраняет подход классических MMORPG:

1. игрок выбирает цель;
2. интерфейс показывает рамку выбранной цели;
3. каждое нажатие кнопки атаки запрашивает одну базовую атаку в ритме `удар → перезарядка → удар`;
4. умения применяются к выбранной цели;
5. учитываются дистанция, перезарядка и ресурс;
...
Перезарядку базовой атаки определяет сервер. Короткий буфер ввода перед её окончанием помогает следующему ручному нажатию сработать отзывчиво, но не позволяет клиенту ускорять ритм.

В бою с существами допускается отдельная необязательная автоматическая атака как режим удобства поверх того же серверного боевого порядка. Ручные атаки остаются основным режимом. Бой между игроками не входит в v0.1; будущий контракт такого боя не допускает автоматическую атаку и требует отдельного ручного нажатия для каждой базовой атаки.

Обычный свободный прыжок не нужен: мир проектируется для наземного перемещения по дорогам, склонам и лестницам. Возможные контекстные действия перемещения относятся к отдельной будущей системе. На мобильных устройствах стандартная кнопка прыжка должна быть убрана, а главным действием справа становится крупная собственная кнопка атаки.
...
### 6.1. Ранний кузнечный цикл

Кузнец делит каталог на Рыцаря, Следопыта, Мистика и обработку материалов. Все классовые каталоги доступны для просмотра, а требования уровня проверяются сервером. Маршрут до уровня 6 связывает добычу волков, пауков и гоблинов с цепочками `сырьё → обработанный материал → No-Grade оружие/броня`; ранние рецепты не требуют ресурсов Cemetery, Fallen Shrine или Ancient Approach. Торговец оставляет consumables и свиток возврата, но не продаёт сырьё, обработанные материалы или готовые классовые наборы.
...
- level 1–5 знакомит с базовыми системами и ранними полевыми угрозами;
- level 5–10 проводит через Goblin Camp / Spider Hollow и Dark Woodland;
- level 10–14 ведёт через Cemetery / Fallen Shrine к Ancient Approach;
- level 15 является финальной ступенью vertical slice и связан с Ruins of Selene / Selene's Fallen Guardian.
...
- после смерти персонаж **не возрождается автоматически**;
- игрок получает выбор «Вернуться в ближайший город? Да / Нет»;
- `Нет` оставляет персонажа мёртвым на месте, при этом возврат можно запросить позднее;
- `Да` и принудительный Reset Character используют один серверный respawn flow;
- сервер сам выбирает ближайшую допустимую точку поселения/телепорта и возвращает туда персонажа;
- полные HP и классовый ресурс восстанавливаются при возрождении;
- без потери level;
...
- player HP/MP/XP;
- target frame;
- включаемый боевой журнал рядом с чатом: нанесённый/полученный урон, критические удары, лечение, эффекты и причины отказа;
- skill/action bar;
- quest tracker;
- inventory;
