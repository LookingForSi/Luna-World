...
### 2.3.1. Уровни существ и поправка XP по разнице уровней

Каждый mob definition имеет явный integer `level`. Стартовый playground:

- Молодой волк — level 1, base XP 15;
- Волк — level 2, base XP 25;
- Паук — level 3, base XP 35.

Base XP принадлежит конкретному типу существа. Фактическая награда дополнительно зависит от разницы `mobLevel - playerLevel`.

Рабочая tuning-функция:

| Разница уровней | Множитель XP |
|---:|---:|
| mob на 5+ уровней выше | 1.25 cap |
| +4 | 1.20 |
| +3 | 1.15 |
| +2 | 1.10 |
| +1 | 1.05 |
...
| -3 | 0.25 |
| -4 | 0.10 |
| player выше mob на 5+ | 0 |

Награда округляется до целого XP. Если multiplier больше нуля, минимальная награда — 1 XP.

Таким образом level 1 персонаж получает небольшой бонус за более сильного противника (до +25%), level 4 получает с level 1 Молодого волка только 4 XP, а начиная с разрыва в 5 уровней слабый mob XP не даёт вообще.

Это server-authoritative tuning rule и не зависит от клиента.
