...
## 17. Owner traversal correction — river, bridge seams and recovery

По owner-прогону после macro composition pass зафиксированы проблемы реки/моста:

- временная ярко-синяя Part-лента заменена на Roblox Terrain Water с более спокойными параметрами воды;
- ширина water-fill увеличена и собирается короткими перекрывающимися сегментами, чтобы не оставлять серые/пустые щели вдоль русла;
- мост получил широкие грунтовые bank-aprons, более широкие деревянные approaches и дополнительное взаимное перекрытие с deck;
- между terrain и мостом не должно оставаться character-sized seam;
- вдоль реки добавлены invisible recovery-volumes;
- сервер хранит последнюю безопасную grounded-позицию игрока;
- падение в реку или ниже мира возвращает игрока в последнюю безопасную точку, а не обязательно обратно в Luna Village;
- recovery-позиция обновляется только внутри world bounds, вне water hazard и когда Humanoid действительно стоит на поверхности.

Следующий owner gate: повторно пройти мост в обе стороны, намеренно свалиться в воду в 2–3 местах и проверить возврат на ближайшую безопасную сторону без застревания.


## 18. World-lock candidate — финальная рихтовка owner traversal

По полному проходу владельца от Luna Village до Ancient Approach перед интеграцией с gameplay-веткой зафиксирован финальный набор геометрических правок:

- стартовый SpawnLocation развёрнут лицом на север / к выходу из деревни;
- толстые временные fence/wall-полосы у фермы заменены читаемыми стойками и тонкими рейками;
- все authored roads при построении меняют Terrain Grass/LeafyGrass на Ground/Cobblestone по ширине маршрута, чтобы трава не пробивалась сквозь дорогу;
- базовые grounded primitives получили небольшой controlled embed в terrain, чтобы убрать массовые визуальные щели под объектами на склонах;
- длинные стены Goblin Camp разбиты на короткие grounded sections, хижины получили заглублённые foundations;
- Spider Hollow получил физически закрытый Terrain-floor, чтобы в чаше не оставалось endless-fall void;
- distant Selene monoliths теперь grounded независимо каждый по своей точке, а не по общей высоте центрального маркера;
- южный склон Fallen Shrine получил намеренно непроходимую каменную стену и scree, сохраняя обход по маршруту;
- добавлены несколько участков открытого камня / scree на холмах и две широкие песчаные точки доступа к реке;
- river/world-fall recovery предыдущего прохода сохраняется.

Статус: **world-lock candidate**, не финальный PASS. Нужен последний owner traversal после обновления ветки. После подтверждения геометрию мира фиксируем и переносим/интегрируем с актуальной gameplay-веткой без дальнейшей перестройки topology.
